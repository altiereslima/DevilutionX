...
float pixelMoveX = 0;
float pixelMoveY = 0;
bool isPixelMoving = false;
constexpr float PixelMoveSpeed = 2.5f;
constexpr float PixelMoveAccel = 0.2f;
constexpr float PixelMoveDecel = 0.4f;

void UpdatePixelMovement()
{
    AxisDirection dir = GetLeftStickOrDpadDirection(true);
    
    // Convert direction to pixel movement
    float targetX = (dir.x == AxisDirectionX_LEFT) ? -PixelMoveSpeed :
                   (dir.x == AxisDirectionX_RIGHT) ? PixelMoveSpeed : 0;
    float targetY = (dir.y == AxisDirectionY_UP) ? -PixelMoveSpeed :
                   (dir.y == AxisDirectionY_DOWN) ? PixelMoveSpeed : 0;

    // Apply acceleration/deceleration
    if (targetX != 0) {
        pixelMoveX = std::clamp(pixelMoveX + (targetX > 0 ? PixelMoveAccel : -PixelMoveAccel),
            -PixelMoveSpeed, PixelMoveSpeed);
    } else {
        if (pixelMoveX > 0)
            pixelMoveX = std::max(0.0f, pixelMoveX - PixelMoveDecel);
        else if (pixelMoveX < 0)
            pixelMoveX = std::min(0.0f, pixelMoveX + PixelMoveDecel);
    }

    if (targetY != 0) {
        pixelMoveY = std::clamp(pixelMoveY + (targetY > 0 ? PixelMoveAccel : -PixelMoveAccel),
            -PixelMoveSpeed, PixelMoveSpeed);
    } else {
        if (pixelMoveY > 0)
            pixelMoveY = std::max(0.0f, pixelMoveY - PixelMoveDecel);
        else if (pixelMoveY < 0)
            pixelMoveY = std::min(0.0f, pixelMoveY + PixelMoveDecel);
    }

    isPixelMoving = (pixelMoveX != 0 || pixelMoveY != 0);
}
